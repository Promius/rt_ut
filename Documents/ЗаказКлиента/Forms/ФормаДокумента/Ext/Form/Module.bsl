...
Процедура РасшРТ_ЦенаСоСкидкойПриИзменении(Элемент)
	...
	Если ТекущиеДанные.Цена <> ТекущиеДанные.ЦенаСоСкидкой Тогда
		// Округляем цену со скидкой до рублей
		ТекущиеДанные.ЦенаСоСкидкой = Окр(ТекущиеДанные.ЦенаСоСкидкой, 0);
	КонецЕсли;
	
	// Пересчитываем сумму ручной скидки
	ТекущиеДанные.СуммаРучнойСкидки = (ТекущиеДанные.Цена - ТекущиеДанные.ЦенаСоСкидкой) * ТекущиеДанные.КоличествоУпаковок;
	ТекущиеДанные.СуммаРучнойСкидки = Окр(ТекущиеДанные.СуммаРучнойСкидки, 0);
...
Процедура РасшРТ_РассчитатьЦенуСоСкидкой(ТекущиеДанные)
	...
	// Пересчитываем сумму с учетом округленной цены
	ТекущиеДанные.Сумма = ТекущиеДанные.ЦенаСоСкидкой * ТекущиеДанные.КоличествоУпаковок;
	
	// Округляем сумму до рублей
	ТекущиеДанные.Сумма = Окр(ТекущиеДанные.Сумма, 0);
КонецПроцедуры
...
Процедура РасшРТ_ТоварыСуммаПриИзмененииПосле(Элемент)
	ТекущиеДанные = Элементы.Товары.ТекущиеДанные;
	ТекущиеДанные.Сумма = Окр(ТекущиеДанные.Сумма, 0);
	РасшРТ_РассчитатьЦенуСоСкидкой(ТекущиеДанные);
КонецПроцедуры
...
&НаКлиенте
Процедура РасшРТ_ПослеЗаписиПосле(ПараметрыЗаписи)
	РасшРТ_РассчитатьЦенуСоСкидкойВСтроках();
КонецПроцедуры


&НаКлиенте
Процедура РасшРТ_ТоварыКоличествоУпаковокПриИзмененииПосле(Элемент)
	РасшРТ_РассчитатьЦенуСоСкидкой(Элементы.Товары.ТекущиеДанные);
КонецПроцедуры
// _)